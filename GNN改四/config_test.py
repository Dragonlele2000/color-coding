...
epoch_per_test = 100  # 训练多少轮输出一次测试结果
epoch_per_print = 10  # 训练多少轮输出一次参数
epoch_change_lr = 10  # 训练多少轮改变一次学习率
learning_rate = 1e-3  # 初始学习率
min_rate = 1e-4  # 最小学习率
use_leaky_relu = not True  # 是否用leaky_relu代替relu
use_elu = not True  # 是否用elu代替relu
use_celu = not True  # 是否用celu代替relu
use_tanh = not True
alpha = 1e-8  # leaky_relu/elu的参数
use_linear = not True  # 线性操作是否不固定
fix_root = True  # 是否固定root
...
batch_size = 32  # 批量训练大小,1
shuffle = not True  # 训练时是否对每轮进行打乱

GPU = True  # 要不要放到GPU上
use_max = not True  # 用max代替add
use_avg = not True  # 每一层对x进行一定的缩放，未完成
avg_ratio = 100  # 缩放比例
