...
def result_rel(k):
    file_result_rel = 'Analyze\\' + str(k) + '-conv1.lin_rel.weight.txt'
    m1 = np.loadtxt(file_result_rel)
    return m1
def result_root(k):
    file_result_root = 'Analyze\\' + str(k) + '-conv1.lin_root.weight.txt'
    m2 = np.loadtxt(file_result_root)
    return m2
...
def main():
    ...
    model = GConv()
    model.conv1.lin_rel.weight.data = torch.FloatTensor(result_rel(k))
    model.conv1.lin_root.weight.data = torch.FloatTensor(result_root(k))
