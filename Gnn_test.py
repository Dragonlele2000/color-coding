...
class GConv(torch.nn.Module):
    def __init__(self):
        ...
        # self.m = BatchNorm1d(num_node_features,momentum=1)
...
def train(model, train_loader, optimizer, criterion):
    ...
    if count % print_interval == 0:
        print(f'loss: {avg_loss / len(train_loader):06f}')
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        if print_param:
            for i in model.named_parameters():
                ...
                parameters = np.round(parameters, decimals=1)

                print(parameters)

                np.savetxt(file, parameters, fmt="%.1f")
