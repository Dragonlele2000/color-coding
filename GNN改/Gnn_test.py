...
class GConv(torch.nn.Module):
    def __init__(self):
        ...
        if use_linear:
            self.linear = Linear(num_node_features * k, num_node_features, bias=False)

        if use_max:
            self.conv1 = GraphConv(num_node_features, num_node_features * k, aggr='max', bias=False)
        else:
            self.conv1 = GraphConv(num_node_features, num_node_features * k, aggr='add', bias=False)
        # torch.nn.init.normal_(self.conv1.lin_rel.weight, 0, 0.05)
        # torch.nn.init.normal_(self.conv1.lin_root.weight, 0, 0.05)
        if use_leaky_relu:
            torch.nn.init.kaiming_uniform_(self.conv1.lin_rel.weight, negative_slope, mode='fan_in')
            torch.nn.init.kaiming_uniform_(self.conv1.lin_root.weight, negative_slope, mode='fan_in')
    ...
    def forward(self, x, edge_index, batch):
        ...
        for i in range(k-1):
            ...
            if use_linear:
                ...
                blocks = torch.chunk(x, k, dim=1)
                # print(blocks)
                x = blocks[0]
                for sub_block in blocks[1:]:
                    x = torch.add(x, sub_block)
        # x = self.lin(x)
...
class my_loss(nn.Module):
    ...
    def forward(self, x, y):
        return torch.mean(torch.pow(torch.abs(x - y), k-1))
...
def main():
    ...
    for epoch in range(1, num_epoch + 1):
        ...
        if print_param and epoch % epoch_per_print:
